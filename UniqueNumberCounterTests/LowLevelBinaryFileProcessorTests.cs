...
namespace UniqueNumberCounterTests
{
    ...
    public class LowLevelBinaryFileProcessorTests : IClassFixture<TestCleanupFixture>
    {
        ...
        public void ProcessBinaryFile_EmptyFile()
        {
            ...
            var processor = new LowLevelBinaryFileProcessor();
        ...
        public void ProcessBinaryFile_VariousDatasetSizes(int rowCount)
        {
            ...
            File.AppendAllText(LogFilePath, $"Temporary file path for {rowCount} is: {tempFilePath}{newLine}");
            stopwatch.Stop();
            File.AppendAllText(LogFilePath, $"File creation duration for {rowCount} numbers: {stopwatch.ElapsedMilliseconds} ms{newLine}");

            stopwatch.Restart();
            var processor = new LowLevelBinaryFileProcessor();
            var (uniqueNumbers, numbersSeenOnce) = processor.ProcessBinaryFile(tempFilePath);
            stopwatch.Stop();
            File.AppendAllText(LogFilePath, $"File processing duration for {rowCount} numbers: {stopwatch.ElapsedMilliseconds} ms{newLine}");
